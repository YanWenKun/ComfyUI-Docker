...
* Decoupling Custom Nodes from ComfyUI in file-level. Now user-installed Custom Nodes and their dependencies are stored in bind mounts (Or Docker volumes), so they won't be affected by image updates.
...
* If you encounter issues with nested mounts, remove the bind mounts for `storage-cache` and the ephemeral `--rm` flag.

** Or just upgrade OS / Docker to a modern version.
