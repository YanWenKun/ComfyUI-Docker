...
if [ ! -f "/root/user-scripts/set-proxy.sh" ] ; then
    ...
    cp /tester-scripts/set-proxy.sh.example /root/user-scripts/set-proxy.sh
else
...
if [ ! -f "/root/.download-complete" ] ; then
    chmod +x /tester-scripts/download.sh
    bash /tester-scripts/download.sh
fi ;
...
if [ ! -f "/root/user-scripts/pre-start.sh" ] ; then
    mkdir -p /root/user-scripts
    cp /tester-scripts/pre-start.sh.example /root/user-scripts/pre-start.sh
else
