...
* Pre-installed:
** SageAttention 2.2.0
*** Compiled using this image.
*** `TORCH_CUDA_ARCH_LIST='8.0;8.6;8.9'`, supports Ampere and Ada GPUs.
** SpargeAttention
*** Compiled using this image.
*** `TORCH_CUDA_ARCH_LIST='8.0;8.6;8.9;9.0'`, supports Ampere and Ada GPUs. May work on Hopper.
...
NOTE: Only one attention implementation can be selected at a time, they are mutually exclusive.
If none is specified, xFormers is enabled by default.

TIP: For Blackwell GPUs, you may want to use a combination like `--use-flash-attention --disable-xformers`.
