...
which uses FFmpeg4. The image came with latest FFmpeg6 or newer.)
...
However, this image's script only install one custom node (ComfyUI-Manager). In a new deployment, you can safely install WAS NS via ComfyUI-Manager. If anything conflicts, just delete unwanted custom nodes and `.local` folder, and update/try fix/reinstall custom nodes in ComfyUI-Manager.
====
