...
TIP: 如果在 ComfyUI 启动、尚无负载时 PyTorch 报错
`UR_RESULT_ERROR_OUT_OF_DEVICE_MEMORY`，一般是兼容性问题，建议改用其他版本的 Linux 内核。


## 运行

TIP: 如连接 Docker Hub 困难，可以尝试先从
https://status.daocloud.io/status/docker[镜像站点]
拉取镜像，再启动容器。
...
|--async-offload
|“将模型权重从显存卸载到内存”这一操作从同步变为异步，带来少量性能提升。对少量 VAE 有兼容问题。
